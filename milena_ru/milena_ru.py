...
class TimerManager:
    ...
    def tell_remaining_timer(self):
        ...
        if remaining <0:
            remaining = 0
        self.speak(f"Осталось {self.human_time(remaining)}.")
...
class MilenaAssistant:
    ...
    def parse_time_from_text(self, command: str) -> int | None:
        ...
        if m2:
            ...
            number = self.words_to_num(words, config_ru.RUS_NUMBERS)
    ...
    #run
    def run(self):
        print(config_ru.WELCOME_MESSAGE)
        ...
        #self.recorder = PvRecorder(device_index=self.device or 0, frame_length=self.porcupine.frame_length)
                ...
                if keyword_index >= 0:
                    ...
                    if matched_cmd and score >= 70:
                        ...
                        if matched_cmd == "gpt_search" and matched_cmd in self.modules_enable and self.modules_enable[matched_cmd]:
                            ...
                            # обычные action-методы, без аргументов
                            if callable(action):
                        ...
                        print(f"Вы ещё не добавили таких функций: {command}")
                        self.speak_async(f"Вы ещё не добавили таких функций: {command}")
